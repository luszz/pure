...
`pure-lint-cli` 是[前端编码规范工程化](https://luszz.github.io/pure/)的配套 Lint 工具，可以为项目一键接入规约、一键扫描和修复规约问题，保障项目的编码规范和代码质量。
...
`pure-lint-cli` 收敛屏蔽了这些依赖和配置细节，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡口，降低项目接入规约的成本。
...
在项目根目录执行 `pure-lint-cli init`，即可一键接入规约，为项目安装规约 `Lint` 所需的依赖和配置。
...
- 配置 git commit 卡口：使用 [husky](https://www.npmjs.com/package/husky) 设置代码提交卡口，在 git commit 时会运行 `pure-lint-cli commit-file-scan` 和 `pure-lint-cli commit-msg-scan` 分别对提交文件和提交信息进行规约检查。`pure-lint-cli commit-file-scan` 默认仅对 error 问题卡口，如果你想对 warn 问题也卡口，可以增加 `--strict` 参数以开启严格模式
...
- pure-lint-cli.init(config)：将项目一键接入规约，效果等同于 `pure-lint-cli init`
